...
void DS3234::enableDisableAlarm(int alarmNr, bool enable) {
	uint8_t controlRegister;

	controlRegister = spiReadByte(DS3234_REGISTER_CONTROL); // // 0x0E
	if (alarmNr == 0)
	{
		if (enable)
		{
			controlRegister |= 0x01; // 0x01 = 0000 0001 = 0000 0001
		}
		else
		{
			controlRegister &= ~0x01; // ~0x01 = ~0000 0001 = 1111 1110
		}
	}
	else
	{
		if (enable)
		{
			controlRegister |= 0x02; // 0x02 = 0000 0010 -> zweite bit wird gesetzt
		}
		else
		{
			controlRegister &= ~0x02; // ~0x02 = 1111 1101 -> zweite Bit wird 0 gesetzt
		}
	}
	spiWriteByte(DS3234_REGISTER_CONTROL, controlRegister);
}
